...
const getAllCoupons = (filter: string, pageNumber: number, pageSize: number, companyId: number, maxPrice: number, category: string, searchedTitle: string) => {
  return axios.get( BASE_URL + `/coupons?filter=${filter}&pageNumber=${pageNumber}&pageSize=${pageSize}&companyId=${companyId}&maxPrice=${maxPrice}&category=${category}&searchedTitle=${searchedTitle}`)
};
